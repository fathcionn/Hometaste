...
function dishCard(dish) {
  ...
  return `
    <article class="card dish-card">
      ...
      <div class="dish-body">
        <h3>${dish.name}</h3>
        ${dish.verified ? `<span class="verified-badge">📸 Live Verified</span>` : ""}
...
function showPhotoPreview(src, isCamera) {
  ...
  if (!preview) return;
  preview.innerHTML = `<img src="${src}" alt="Photo preview">${isCamera ? `<span class="verified-badge">📸 Live Verified</span>` : ""}`;
}
