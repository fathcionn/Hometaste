...
function bindHeaderControls() {
  ...
  document.querySelector("#themeToggle")?.addEventListener("click", () => {
    ...
    applyTheme(next);
    api("/api/settings", { method: "PATCH", body: JSON.stringify({ theme: next }) }).catch(() => {});
  });
  ...
  document.querySelectorAll("[data-header-lang]").forEach((button) => {
    button.addEventListener("click", (event) => {
      ...
      setLang(lang);
      api("/api/settings", { method: "PATCH", body: JSON.stringify({ language: lang }) }).catch(() => {});
...
function renderAdmin() {
  ...
  return `
    ${header("Admin Control", "Review real cook applications, dish submissions, users, and stored orders.")}
    <section class="grid cols-4">
    ...
    <section class="grid cols-2" style="margin-top:18px">
      <div class="panel">
        <h3>Pending cook applications</h3>
        ${(state.cookApplications || []).filter((app) => app.status === "pending").length ? (state.cookApplications || []).filter((app) => app.status === "pending").map((app) => `
          <div class="row">
            <div><strong>${app.name}</strong><div class="meta">${app.cuisine} in ${app.city}, ${app.country} - ${app.phone || "No phone"}</div><div class="meta">${app.bio || "No bio"}</div></div>
            <div class="toolbar" style="margin:0">
              <button class="button small good" data-application-status="${app.id}" data-status="approved">Approve</button>
              <button class="button small bad" data-application-status="${app.id}" data-status="rejected">Reject</button>
            </div>
          </div>
        `).join("") : `<div class="empty">No pending cook applications.</div>`}
      </div>
      <div class="panel">
        <h3>Pending dishes</h3>
        ${state.dishes.filter((dish) => dish.status === "pending").length ? state.dishes.filter((dish) => dish.status === "pending").map((dish) => `
          <div class="row">
            <div><strong>${dish.name}</strong><div class="meta">${cookName(dish.cookId)} - ${money(dish.price)} - pending approval</div></div>
            <div class="toolbar" style="margin:0">
              <button class="button small good" data-admin-dish="${dish.id}" data-status="approved">Approve</button>
              <button class="button small bad" data-admin-dish="${dish.id}" data-status="rejected">Reject</button>
            </div>
          </div>
        `).join("") : `<div class="empty">No pending dishes.</div>`}
      </div>
    </section>
    <section class="grid cols-2" style="margin-top:18px">
      <div class="panel">
        ...
        ${state.cooks.map((cook) => `
          <div class="row">
            <div><strong>${cook.name}</strong><div class="meta">${cook.cuisineSpecialty || cook.cuisine || "Home Kitchen"} in ${cook.city} - <span class="status">${cook.status}</span></div></div>
            <div class="toolbar" style="margin:0">
      ...
      <div class="panel">
        ...
        ${state.dishes.map((dish) => `
          <div class="row">
            <div><strong>${dish.name}</strong><div class="meta">${cookName(dish.cookId)} - ${money(dish.price)} - ${dish.status || "pending"}</div></div>
            <div class="toolbar" style="margin:0">
              <button class="button small good" data-admin-dish="${dish.id}" data-status="approved">Approve</button>
              <button class="button small bad" data-admin-dish="${dish.id}" data-status="rejected">Reject</button>
            </div>
      ...
      <table class="table">
        ...
        <tbody>${state.users.map((user) => {
          ...
          <tr>
            ...
            <td>${cook ? `${cook.name}<div class="meta">${cook.cuisineSpecialty || cook.cuisine || "Home Kitchen"} - ${cook.status} - ${cook.verifiedCook ? "approved" : "not approved"}</div>` : `<span class="meta">Eater account</span>`}</td>
            <td>
...
function bindPage() {
  ...
  document.querySelectorAll("[data-application-status]").forEach((button) => {
    button.onclick = () => applicationStatus(button.dataset.applicationStatus, button.dataset.status);
  });
  document.querySelectorAll("[data-admin-dish]").forEach((button) => {
    button.onclick = () => adminDishStatus(button.dataset.adminDish, button.dataset.status);
  });
...
async function createDish(event) {
  ...
  const input = Object.fromEntries(new FormData(event.currentTarget).entries());
  try {
    state = await api("/api/dishes", { method: "POST", body: JSON.stringify({ ...input, image: currentPhotoData || input.image, cameraVerified: currentPhotoVerified, cameraCapture: currentPhotoVerified }) });
    toast(t("toast_saved"));
    renderApp();
  } catch (err) {
    toast(err.message, true);
...
async function applicationStatus(applicationId, status) {
  try {
    const data = await api(`/api/admin/applications/${applicationId}`, { method: "PATCH", body: JSON.stringify({ status }) });
    state = data.state || data;
    toast(status === "approved" ? "Cook application approved." : "Cook application rejected.");
    renderApp();
  } catch (err) {
    toast(err.message, true);
  }
...
async function adminDishStatus(dishId, status) {
  try {
    const data = await api(`/api/admin/dishes/${dishId}`, { method: "PATCH", body: JSON.stringify({ status }) });
    state = data.state || data;
    toast(status === "approved" ? "Dish approved." : "Dish rejected.");
    renderApp();
  } catch (err) {
    toast(err.message, true);
  }
