...
function publicState(db, user = null) {
  ...
  return {
    ...
    cookApplications: isAdmin(user) ? db.cookApplications : [],
    reports: isAdmin(user) ? db.reports : [],
...
async function api(req, res, pathname) {
  ...
  if (req.method === "POST" && pathname === "/api/auth/signup") {
    ...
    const requestedCook = input.role === "cook";
    const role = "customer";
    const user = {
      id: id("usr"),
      name,
    ...
    db.users.push(user);
    if (requestedCook) {
      db.cookApplications.push({
        id: id("app"),
        userId: user.id,
        name,
        cuisine: String(input.cuisine || input.countryOrigin || "Home Kitchen").trim(),
        city: user.city,
        country: String(input.country || input.countryOrigin || "Turkey").trim(),
        phone: user.phone,
  ...
  if (req.method === "GET" && pathname === "/api/messages") {
    const mine = db.messages
      .filter((message) => message.senderId === user.id || message.receiverId === user.id || isAdmin(user))
      .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
    return json(res, 200, { messages: mine, total: mine.length });
  }
  ...
  if (req.method === "GET" && pathname.startsWith("/api/messages/")) {
    ...
    if (!mine.length) return json(res, 404, { error: "Conversation not found." });
    return json(res, 200, { messages: mine, conversationId });
  }
  ...
  if (req.method === "POST" && pathname === "/api/orders") {
    ...
    try {
      ...
      if (!Number.isFinite(quantity) || quantity < 1) throw new Error("Quantities must be positive.");
      const dish = db.dishes.find((d) => d.id === item.dishId && d.status === "approved");
      if (!dish) throw new Error("This dish is no longer available.");
      const allowedExtras = [...(dish.sauces || []), ...(dish.drinks || [])].map((extra) =>
        typeof extra === "string" ? { id: extra, name: extra, price: 0 } : extra
      );
      const submittedExtras = Array.isArray(item.extras) ? item.extras : [];
      const extras = submittedExtras.map((extra) => {
        const match = allowedExtras.find((allowed) => allowed.id === extra.id || allowed.name === extra.name);
        if (!match) throw new Error(`Extra "${extra.id || extra.name || "unknown"}" does not belong to dish "${dish.id}".`);
        return { id: match.id || match.name, name: match.name || match.id, price: Number(match.price || 0) };
      });
      const extrasTotal = extras.reduce((sum, extra) => sum + Number(extra.price || 0), 0);
      return { dishId: dish.id, name: dish.name, quantity: Math.max(1, Number(item.quantity || item.qty || 1)), unitPrice: dish.price + extrasTotal, basePrice: dish.price, extras };
    });
    } catch (error) {
      return json(res, 400, { error: error.message || "Invalid cart item." });
    }
  ...
  if (isAdmin(user) && req.method === "GET" && pathname === "/api/admin/applications") {
    return json(res, 200, { applications: db.cookApplications, total: db.cookApplications.length });
  }

  if (isAdmin(user) && req.method === "PATCH" && pathname.startsWith("/api/admin/applications/")) {
    const application = db.cookApplications.find((item) => item.id === pathname.split("/").pop());
    if (!application) return json(res, 404, { error: "Application not found." });
    const input = await body(req);
    const status = String(input.status || "").trim();
    if (!["approved", "rejected"].includes(status)) return json(res, 400, { error: "Application status must be approved or rejected." });
    application.status = status;
    application.reviewedAt = now();
    application.reviewedBy = user.id;
    if (input.reason) application.reason = String(input.reason).trim();
    if (status === "approved") {
      const targetUser = db.users.find((item) => item.id === application.userId);
      if (targetUser) targetUser.role = "cook";
      let cook = db.cooks.find((item) => item.userId === application.userId);
      if (!cook) {
        cook = {
          id: id("cook"),
    ...
    await saveDb(db);
    return json(res, 200, { application, state: publicState(db, user) });
  }

  if (isAdmin(user) && req.method === "GET" && pathname === "/api/admin/dishes") {
    const status = requestUrl.searchParams.get("status");
    const list = db.dishes.filter((dish) => !status || dish.status === status).map((dish) => publicDish(db, dish));
    return json(res, 200, { dishes: list, total: list.length });
  }

  if (isAdmin(user) && req.method === "PATCH" && pathname.startsWith("/api/admin/dishes/")) {
    const dish = db.dishes.find((item) => item.id === pathname.split("/").pop());
    if (!dish) return json(res, 404, { error: "Dish not found." });
    const input = await body(req);
    if (!["approved", "pending", "rejected"].includes(input.status)) return json(res, 400, { error: "Dish status must be approved, pending, or rejected." });
    dish.status = input.status;
    dish.reviewedAt = now();
    dish.reviewedBy = user.id;
    await saveDb(db);
    return json(res, 200, { dish: publicDish(db, dish), state: publicState(db, user) });
  }
